...
from example_feature_repo.example import driver, driver_hourly_stats_view
...
def test_cli():
    ...
    os.system(
        f"PYTHONPATH=$PYTHONPATH:/$(pwd) feast -c {repo_name} apply"
    )
    try:
        os.system(
        ...
        os.system(
            f"PYTHONPATH=$PYTHONPATH:/$(pwd) feast -c {repo_name} teardown"
        )
